...
class Problem(object):
    """
    An abstract base class to define integration problems for validation testing
    """
    ...
    with open('logfile', 'a') as file:
        for f in glob(pjoin('log', globtxt)):
            ...
            file.write('L2 (max, mean) = {:.16e}, {:.16e}\n'.format(
                np.max(L2_err), np.mean(L2_err)))
            file.write('Linf (max, mean) = {:.16e}, {:.16e}\n'.format(
                np.max(Linf_err), np.mean(Linf_err)))
            ...
            for spec, ind in minor_species:
                ...
                file.write('{} L2 (max, mean) = {:.16e}, {:.16e}\n'.format(
                    spec, np.max(L2_err), np.mean(L2_err)))
                file.write('{} Linf (max, mean) = {:.16e}, {:.16e}\n'.format(
                    spec, np.max(Linf_err), np.mean(Linf_err)))
    ...
    with open('logerr', 'a') as file:
        if builder == 'gpu':
            for exe in glob('*-gpu'):
                ...
                try:
                    subprocess.check_call(
                        [pjoin(cwd(), exe), str(num_conditions)], stdout=file)
                except subprocess.CalledProcessError as e:
                    returncode = e.returncode
                    file.write('Error encountered running {}\n'.format(
                        ' '.join([exe, str(num_conditions)])))
            ...
            for exe in glob('*-int'):
                ...
                file.write('\n' + exe + '\n')
                try:
                    subprocess.check_call([pjoin(cwd(), exe), str(
                        num_threads), str(num_conditions)], stdout=file)
                except subprocess.CalledProcessError as e:
                    returncode = e.returncode
                    file.write('Error encountered running {}\n'.format(
                        ' '.join([exe, str(num_threads), str(num_conditions)])))
                    file.write('Error code: {}\n'.format(returncode))
                    sys.exit(-1)
                shutil.copy(pjoin('log', exe + '-log.bin'),
                            pjoin('log', exe + '-log_{:.0e}.bin'.format(t_step)))
...
def __run_and_check(mech, thermo, initial_conditions, build_path,
    ...
    if num_conditions is None and test_data is not None:
        num_conditions = np.fromfile(
            'ign_data.bin').reshape((-1, nvar + 2)).shape[0]
    ...
    small_tol = ['ATOL={:.0e}'.format(
        small_atol), 'RTOL={:.0e}'.format(small_rtol)]
    large_tol = ['ATOL={:.0e}'.format(atol), 'RTOL={:.0e}'.format(rtol)]
    # build the validation set for this timestep
    extra_args = ['t_step={:.0e}'.format(
        small_time_step), 't_end={:.0e}'.format(end_time)]

    if not use_old_validation:
        with open('logerr', 'a') as errfile:
            subprocess.check_call(
                [scons, 'cpu'] + arg_list + extra_args + small_tol, stdout=errfile)
            # run
...
if __name__ == '__main__':
    parser = ArgumentParser(
        description='logger: Log and compare solver output for the various ODE Solvers')

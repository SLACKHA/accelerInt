...
void dydt ( Real t, Real pres, Real * y, Real * dy ) {
  ...
  Real cp_avg = ( cp[0] * y[1] ) + ( cp[1] * y[2] ) + ( cp[2] * y[3] ) + ( cp[3] * y[4] )
              ...
              + ( cp[8] * y[9] ) + ( cp[9] * y[10] ) + ( cp[10] * y[11] )
              + ( cp[11] * y[12] ) + ( cp[12] * y[13] );
  ...
  dy[1] = dy[1] * 1.00797 / rho;
  dy[2] = dy[2] * 2.01594 / rho;
  dy[3] = dy[3] * 15.9994 / rho;
  dy[4] = dy[4] * 17.00737 / rho;
  dy[5] = dy[5] * 18.01534 / rho;
  dy[6] = dy[6] * 31.9988 / rho;
  dy[7] = dy[7] * 33.00677 / rho;
  dy[8] = dy[8] * 34.01474 / rho;
  dy[9] = dy[9] * 28.0134 / rho;
  dy[10] = dy[10] * 39.948 / rho;
  dy[11] = dy[11] * 4.0026 / rho;
  dy[12] = dy[12] * 28.01055 / rho;
  dy[13] = dy[13] * 44.00995 / rho;

} // end dydt

#elif defined(CONV)

void dydt ( Real t, Real rho, Real * y, Real * dy ) {
  ...
  pres = ( y[1] / 1.00797 ) + ( y[2] / 2.01594 ) + ( y[3] / 15.9994 ) + ( y[4] / 17.00737 )
       + ( y[5] / 18.01534 ) + ( y[6] / 31.9988 ) + ( y[7] / 33.00677 )
       + ( y[8] / 34.01474 ) + ( y[9] / 28.0134 ) + ( y[10] / 39.948 ) + ( y[11] / 4.0026 )
       + ( y[12] / 28.01055 ) + ( y[13] / 44.00995 );
  pres = rho * 8.31451000e+07 * T * pres;
  ...
  Real rates[27];
  eval_rxn_rates ( T, pres, conc, rates );
  ...
  Real cv_avg = ( cv[0] * y[1] ) + ( cv[1] * y[2] ) + ( cv[2] * y[3] ) + ( cv[3] * y[4] )
              ...
              + ( cv[8] * y[9] ) + ( cv[9] * y[10] ) + ( cv[10] * y[11] )
              + ( cv[11] * y[12] ) + ( cv[12] * y[13] );
